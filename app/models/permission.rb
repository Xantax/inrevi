class Permission < Struct.new(:user)
  def allow?(controller, action)
    ...
    return true if controller == "podcast_reviews" && action.in?(%w[index show])
    return true if controller == "tech_reviews" && action.in?(%w[index show])
    return true if controller == "movie_reviews" && action.in?(%w[index show])
    return true if controller == "tvshow_reviews" && action.in?(%w[index show])
    return true if controller == "song_reviews" && action.in?(%w[index show])
    return true if controller == "local_reviews" && action.in?(%w[index show])
    return true if controller == "book_reviews" && action.in?(%w[index show])
    if user
